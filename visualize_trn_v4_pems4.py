...
SEED = 6789
graph_dir = '/home/s223719687/python_project/traffic_evolve/data/PeMS4_Expand/PeMS4_Expand/graph'
...
meta_df = pd.read_csv('/home/s223719687/python_project/traffic_evolve/cl_traffic_coat/sensor4_lat_long_v2.csv', sep='\t')
meta_df.loc[:,'index'] = meta_df.loc[:,'index'] - 1

bad_sensor_index = [5, 17, 8, 7, 12, 20, 96, 10, 15, 16, 14, 4, 0, 9, 40, 13, 33, 59, 29, 2, 18, 64, 67, 70, 69, 74, 49, 41, 66, 71, 79, 81, 84, 89, 91, 96, 99, 100, 76, 111, 57]
...
def generate_sensor_map(traffic_sensors_df, nCount, new_node_list, removed_node_list, year):
    """
    ...
    # Create a base map
    m = folium.Map(location=[centroid_lat, centroid_lon], zoom_start=11.1)  # Centered around San Francisco
    ...
    for node_index in range (0, nCount):
        if (node_index in bad_sensor_index):
            continue
...
for year in range(2011, 2018):
    ...
    if year == 2011:
        ...
        remove_nodes = np.load(f'/home/s223719687/python_project/traffic_evolve/data/PeMS4_Expand/PeMS4_Expand/FastData_removed/{year}_remove_id.npz')['id']
        remove_edges.extend(graph.edges(remove_edges))
        
        nCount_draw = prev_nCount
    
    map_output = generate_sensor_map(meta_df, nCount_draw, new_nodes, remove_nodes, year)
    
    img_data = map_output._to_png(3)
    
    img = Image.open(io.BytesIO(img_data))
    img.save(f'fig/pems04/test3/{year}.png')
